...
export default function DashboardSidebar({ totalCandidates, recentJobs }: { totalCandidates: number, recentJobs: any[] }) {
  return (
          ...
          {recentJobs && recentJobs.length > 0 ? (
            recentJobs.map((job: any) => (
              <div key={job._id || job.id} className={`${job.status === 'Urgently Hiring' ? 'bg-orange-400' : 'bg-indigo-600'} p-5 rounded-2xl text-white shadow-lg shadow-indigo-100/20`}>
                ...
                <div className="text-[10px] mt-1 font-medium opacity-70">Total Applicants - {totalCandidates}</div>
              </div>
            ))
          ) : (
