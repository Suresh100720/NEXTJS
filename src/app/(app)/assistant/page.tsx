...
import {
  ...
  Terminal, Code2, FileText, Mail, FileCode, Check, Bot, User,
  Zap, Activity, ChevronDown, ChevronRight
} from 'lucide-react';

// ─── Types ────────────────────────────────────────────────────────────────────
type ToolName = 'searchElasticsearch' | 'queryMongo';

// ─── Tool Icon component ───────────────────────────────────────────────────────
function ToolCard({ invocation }: { invocation: any }) {
  const [open, setOpen] = useState(false);
  const { toolName, toolCallId, state } = invocation;
  const isES = toolName === 'searchElasticsearch';
  const done = state === 'result';

  return (
    <div className="mt-3 rounded-xl border border-slate-300 bg-slate-50 overflow-hidden text-xs font-mono">
      {/* Header row */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-white border-b border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <span className={`font-bold ${isES ? 'text-sky-600' : 'text-violet-600'}`}>
            {toolName}
          </span>
          <span className="text-slate-400">·</span>
          <span className="text-slate-500 text-[10px]">{toolCallId.slice(0, 16)}…</span>
        </div>
      ...
      {/* Args */}
      <div className="px-4 py-2 text-[11px] text-slate-600 border-t border-slate-200">
        <span className="text-slate-500">args → </span>
        <span className="text-slate-700">{JSON.stringify(invocation.args)}</span>
      </div>
      ...
      {done && open && (
        <div className="border-t border-slate-200 px-4 py-3">
          <p className="text-[10px] font-black uppercase tracking-wider text-indigo-600 mb-2 flex items-center gap-1">
            <Code2 className="w-3 h-3" /> Response JSON
          </p>
          <pre className="text-[10px] text-slate-700 whitespace-pre overflow-x-auto max-h-48 leading-relaxed">
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 min-w-0">
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white px-4 py-3">
          ...
          >
            ...
            <button
              type="submit"
              disabled={chatLoading || !safeInput.trim()}
      ...
      <div className="w-[440px] shrink-0 flex flex-col bg-slate-50 border-l border-slate-200">
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
          ...
          <div className="flex-1 flex flex-col min-h-[200px]">
            ...
            <div className="flex-1 bg-white border border-slate-300 rounded-xl p-3 overflow-y-auto text-xs text-slate-700 leading-relaxed whitespace-pre-wrap font-mono">
              {completion || (
